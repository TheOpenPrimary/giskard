			...
			def send_msg(id,msg,options)
				...
				lines.each do |l|
					...
					if image && !buffer.empty? then # flush buffer before sending image
						...
						Giskard::TG::Messenger.client.api.send_chat_action(chat_id: id, action: "typing")
						sleep(writing_time)
						Giskard::TG::Messenger.client.api.sendMessage(chat_id: id, text: buffer)
						buffer=""
					end
					if image then # sending image
						...
						Giskard::TG::Messenger.client.api.send_chat_action(chat_id: id, action: "upload_photo")
						Giskard::TG::Messenger.client.api.send_photo(chat_id: id, photo: img)
						...
						if (idx==max) then # flush buffer
							...
							Giskard::TG::Messenger.client.api.sendChatAction(chat_id: id, action: "typing")
							sleep(writing_time)
							Giskard::TG::Messenger.client.api.sendMessage(chat_id: id, text: buffer, reply_markup:kbd)
							buffer=""
						end
						...
						Giskard::TG::Messenger.client.api.sendMessage(options)
		...
		post '/' do
			begin
				...
				if e.message.match(/blocked/).nil? and e.message.match(/kicked/).nil? then
					Giskard::TG::Messenger.client.api.sendChatAction(chat_id: id, action: "typing")
					Giskard::TG::Messenger.client.api.sendMessage({
						:chat_id=>id,
